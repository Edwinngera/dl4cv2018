...
class CnnClassifier(Model):
    ...
    def __init__(self, net: nn.Module, input_shape: tuple, num_classes: int, lr: float, wd: float, adam: bool=False):
        '''
        Ctor.
    ...
    def train(self, data: np.ndarray, labels: np.ndarray) -> float:
        ...
        if len(data.shape) != 4 or data.shape[1:] != self.input_shape:
            print(data.shape, data.shape[1])
            raise TypeError("data: Expected input shape (m, %s)" % str(self.input_shape))

        if labels.shape != (data.shape[0],):
            raise TypeError("labels: Expected input shape (%s, )" % data.shape[0])

        if not (np.issubdtype(labels.dtype, np.integer) and labels.max() < self.num_classes):
    ...
    def predict(self, data: np.ndarray) -> np.ndarray:
        ...
        if len(data.shape) != 4 or data.shape[1:] != self.input_shape:
            raise TypeError("data: Expected input shape (m, %s)" % self.input_shape)
